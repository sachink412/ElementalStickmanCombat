...
public class ReflectionHelper {
  ...
  static {
    ...
    try {
      // Try to construct the RecordSupportedHelper, if this fails, records are not
      // supported on
      // this JVM.
   ...
   *
   * @param object the object that {@link AccessibleObject#setAccessible(boolean)}
   *               should be called
   *               on.
  ...
  /**
   * Returns a short string describing the {@link AccessibleObject} in a
   * human-readable way. The
   * result is normally shorter than {@link AccessibleObject#toString()} because
   * it omits modifiers
   * (e.g. {@code final}) and uses simple names for constructor and method
   * parameter types.
   *
   * @param object               object to describe
   * @param uppercaseFirstLetter whether the first letter of the description
  ...
  /**
   * Creates a string representation for a field, omitting modifiers and the field
   * type.
   */
  ...
  /**
   * Creates a string representation for a constructor. E.g.:
   * {@code java.lang.String(char[], int,
   * int)}
   */
  ...
  // Ideally parameter type would be java.lang.reflect.Executable, but that was
  // added in Java 8
  ...
  /**
   * Tries making the constructor accessible, returning an exception message if
   * this fails.
   *
  ...
  /**
   * If records are supported on the JVM, this is equivalent to a call to
   * Class.isRecord()
   */
  ...
  /**
   * Looks up the record accessor method that corresponds to the given record
   * field
   */
  ...
  private static class RecordSupportedHelper extends RecordHelper {
    ...
    public <T> Constructor<T> getCanonicalRecordConstructor(Class<T> raw) {
      try {
        ...
        // Uses getDeclaredConstructor because implicit constructor has same visibility
        // as record
    ...
    public Method getAccessor(Class<?> raw, Field field) {
      try {
        // Records consists of record components, each with a unique name, a
