...
public class Intersector {
    ...
    public static Set<ArrayList<?>> getShapeIntersections(final Shape poly1, final Shape poly2) throws Exception {
        ...
        while (!polyIt2.isDone()) {
            ...
            switch (type) {
                case PathIterator.SEG_LINETO: {
                    final Line2D.Double currentLine = new Line2D.Double(lastCoords2[0], lastCoords2[1], coords2[0],
                            coords2[1]);
                ...
                case PathIterator.SEG_CLOSE: {
                    if (!(poly2 instanceof Ellipse2D)) {
                        final Line2D.Double currentLine = new Line2D.Double(coords2[0], coords2[1], firstCoords2[0],
                                firstCoords2[1]);
                        lines2.add(currentLine);
                    }
                    break;
                ...
                case PathIterator.SEG_CUBICTO: {
                    CubicCurve2D curve = new CubicCurve2D.Double(lastCoords2[0], lastCoords2[1], coords2[0], coords2[1],
                            coords2[2], coords2[3], coords2[4], coords2[5]);
        ...
        for (Line2D.Double line : lines) {
            for (Line2D.Double line2 : lines2) {
                if (line.intersectsLine(line2)) {
                    intersections.add(
                            new ArrayList(Arrays.asList(new Object[] { getIntersection(line, line2), line, line2 })));
                }
            }
        }
